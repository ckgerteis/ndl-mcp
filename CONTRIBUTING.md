...
```bash
git clone https://github.com/ckgerteis/ndl-mcp
cd ndl-mcp
python -m venv .venv
...
`python mcpb/build.py` writes `dist/ndl-mcp-<version>.mcpb`. The bundle vendors nothing: its manifest
...
the install step and download everything inside the launch timeout, which is the fault 1.2.2 repaired.
